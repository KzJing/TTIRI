...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if input_ids is not None:
            input_shape = input_ids.size()  # by kzj: input_ids: (b s), input_shape: [b s]
        else:
...
class BertUnpadSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor,
        ...
        # by kzj: hidden_states: (b s hd), qkv: (b s thd), usual hd = 768 t = 3, of course h = 12 and d = 64.
        qkv = rearrange(qkv,
                        'b s (t h d) -> b s t h d',
                        t=3,
                        h=self.num_attention_heads)
        # by kzj: current qkv: (b s t h d) -> (b s 3 12 64) where t = 3, h = 12 and d = 64.
        ...
        # by kzj: unpad_input_only: (b s h d) -> (nnz h d) where nnz = b*s - nz,
        # nnz: number of non-zero in attn_mask, nz: number of zero in attn_mask.
...
class BertUnpadAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.output = BertSelfOutput(config)  # Includes fully connected, regularized loss, and layer normalization (with residuals).
...
# The corresponding script of DNABERT2 is modified
class BertEncoder(nn.Module):
    ...
    def rebuild_alibi_tensor(self,
        ...
        context_position = torch.arange(size, device=device)[:, None]  # : (size, 1) e.g [[0], [1], ..., [size-1]].
        memory_position = torch.arange(size, device=device)[None, :]  # : (1, size) e.g [[0, 1, ..., size-1]]
        ...
        **kwargs
        ...
        if masked_tokens_mask is None:
            ...
            if self.pooler is not None:
                ...
                # The default pool is set to True to return the result of the first token (b 1 h), and False to return (b s h).
...
def mixup(input, alpha):
    ...
    # by kzj: (bs, 1, 1), lam_expanded * -> (bs, seq_len, hs) has the same dimension as input
    lam_expanded = lam.view([-1] + [1]*(input.dim()-1))
    output = lam_expanded * input + (1. - lam_expanded) * input[randind]
    # output: (bs, seq_len, hs), randind: (bs), lam: (bs)
